...
本章的逻辑结构：首先介绍Quarto文档的全局设置，即YAML语法（@sec-yaml设置总览）；然后介绍图片的设置（@sec-图片设置 ），包括插入的图片和代码块运行后产生的图片；随后介绍如何实现对图、表等的交叉引用（@sec-交叉引用 ）；随后介绍其他几种内容的插入（@sec-插入其他内容 ）；随后，介绍创建和编辑Quarto Books的方法（@sec-quarto-books ）；最后，介绍如何将Quarto项目的源代码通过Git上传到GitHub以及如何将编译好的Quarto Book通过GitHub Pages**进行发布（**@sec-发布到github-pages **）。**

...
-   `caption`和`label`的设置会使该图像能够被交叉引用（详见 @sec-交叉引用 ）。
...
其基本语法和 @sec-代码块组图输出设置 一致。不同点在于需要在Div块开头的"{}"内设置，同时`layout`后要接"="，并且注意加引号，例如：`layout="[[1，1]，[1]]"`。通过设置`layout`可以完成对多图的复杂排版。`layout`属性是一个二维数组，其中第一维定义行，第二维定义列。`layout="[[1，1]，[1]]"`表示：创建两行，第一行有两列大小相等的列，第二行只有一列。
...
对于代码块运行后生成的图片，我们也可以对其进行各种设置以使其在编译后的文档中符合理想的展示要求。基本语法和 @sec-插入图片的设置 中类似，只不过需要在前面加上"**#\|**"符号，然后将其放置在代码块开头。常用的参数有：

-   `#| lable`：图片标签。

-   `#| fig-cap`：图片标题（`caption`）。`fig-cap`和`lable`共同用于图片的交叉引用，详见 @sec-交叉引用。

-   `#| fig-width`：图片的宽度。

-   `#| fig-height`：图片的高度。

其他设置包括`#| fig-align`、`#| fig-cap-location`等，见 @sec-图片设置 。
...
### 代码块组图输出设置 {#sec-代码块组图输出设置}

如果一个代码块运行后可以生成多张图像，那么我们也可以和 @sec-组图的设置 中一样，对这些图片进行组图排版。常用的参数包括：
...
#### 用`layout`进行复杂排版 {#sec-用layout进行复杂排版}

...
::: callout-tip
当存在以下情况之一时，最好使用RPCA进行整合：

1.  一个数据集中的很大一部分细胞在另一个数据集中没有匹配类型；

2.  数据集来自同一平台（即10x的多个通道）；
...
Users can also perform integration using sctransform-normalized data (see 基[于SCTransform的单细胞数据标准化](/single_cell/seurat/sctransform.qmd) for more information), by first running `SCTransform` normalization, and then s**etting the `normalization.method` argument in `IntegrateLayers`**（和[执行`SCTransform`标准化流程之后的整合](/single_cell/seurat/integration.qmd#sec-integration_after_sct)中一样）。

...
As an alternative to log-normalization, Seurat also includes support for preprocessing of scRNA-seq using the [`SCTransform()` workflow](/single_cell/seurat/sctransform.qmd). The `IntegrateLayers` function also supports SCTransform-normalized data, by setting the `normalization.method` parameter, as shown below.

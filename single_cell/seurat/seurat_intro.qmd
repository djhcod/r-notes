...
-   **Integration workflow:**\
    Seurat v5 introduces a streamlined integration (见[Seurat v5单细胞数据整合分析](/single_cell/seurat/integrative_analysis_in_seurat_v5.qmd)) and data transfer (见[基于参考集的细胞注释](/single_cell/seurat/mapping_and_annotating_query_datasets.qmd)) workflows that performs integration in low-dimensional space, and improves speed and memory efficiency. The results of integration are not identical between the two workflows, but users can still run the [v4 integration workflow](https://satijalab.org/seurat/articles/integration_introduction) in Seurat v5 if they wish.

    In previous versions of Seurat, the integration workflow required a list of multiple Seurat objects as input. In Seurat v5, **all the data can be kept as a single object**, but prior to integration, users can simply split the layers. See [本章节](/single_cell/seurat/integration.qmd) for more information.
...
-   **Differential expression:**\
    ...
    In addition, in Seurat v5 we implement a **pseudocount** (when calculating log-FC) at the **group level** instead of the cell level. As a result, users will observe **higher logFC estimates** in v5 - but should note that these estimates may be more unstable - particularly for genes that are very lowly expressed in one of the two groups（[寻找不同样本类型间同一细胞类型内的差异基因](/single_cell/seurat/de_vignette.qmd)）. We gratefully acknowledge feedback from the McCarthy and Pachter labs on this topic.

-   **SCTransform v2:**\
    In [@choudhary2022], we implement an updated version 2 of `sctransform`. This is now the default version when running `SCTransform` in Seurat v5 (见[基于SCTransform的单细胞数据标准化](/single_cell/seurat/sctransform.qmd)). Users who wish to run the previous workflow can set the `vst.flavor = "v1"` argument in the `SCTransform` function.

...
The input for this analysis is a `seurat` object. We will use the one that we created in @sec-质控 called `filtered_seurat`.
...
::: callout-tip
在需要消除细胞周期的影响时，如何通过`ScaleData`回归掉（regress out）细胞周期的影响，以及如何在消除细胞周期的影响同时保留增殖细胞与静止细胞的区分，参考 @sec-消除细胞周期的影响 。
:::

...
链接：<https://satijalab.org/seurat/articles/pbmc3k_tutorial>

官方教程发布日期：2023年10月31日
...
[SCTransform()](https://satijalab.org/seurat/reference/sctransform) normalization workflow 就是这样的一种工作流. 针对这一方法的原始文献：[@choudhary2022] 。基于`SCTransform`的数据标准化流程中，无需使用上面的`NormalizeData`, 和下面的`FindVariableFeatures`和`ScaleData`函数。

## 识别高变基因（highly variable features）
